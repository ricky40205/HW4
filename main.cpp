...
int main(){
  ...
  thread_acc.start(background_Acc);
...
void background_Acc(){
  ...
  while(loop < 26){
   ...
   if (acc16 > UINT14_MAX/2)
      acc16 -= UINT14_MAX;
   table[loop][0] = ((float)acc16) / 4096.0f;

   acc16 = (res[2] << 6) | (res[3] >> 2);
   if (acc16 > UINT14_MAX/2)
      acc16 -= UINT14_MAX;
   table[loop][1] = ((float)acc16) / 4096.0f;

   acc16 = (res[4] << 6) | (res[5] >> 2);
   if (acc16 > UINT14_MAX/2)
      acc16 -= UINT14_MAX;
   table[loop][2] = ((float)acc16) / 4096.0f;
   table[loop][3] = (float)timer.read();

   pc.printf("\n\r%1.4f\n\r%1.4f\n\r%1.4f\n\r%1.4f\n\r", table[loop][0], table[loop][1], table[loop][2], table[loop][3]);
...
void getAcc(Arguments *in, Reply *out) {
  //xbee.printf("%1.4f\n%1.4f\n%1.4f\n%1.4f\n", t[0], t[1], t[2]);
  int state = in->getArg<int>();
  if(state == 0) return;

  if(loop == -1){
    loop ++;
    xbee.printf("%d\n", 666);
    pc.printf("start to record\r\n");
    timer.start();
  }
  else if(loop > sample_num){
    xbee.printf("%d\n", -1);
